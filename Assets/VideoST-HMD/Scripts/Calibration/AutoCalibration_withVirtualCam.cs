...
public class AutoCalibration_withVirtualCam : MonoBehaviour
{
    ...
    public GameObject mLeapObject3D;

    // 
    IntPtr image_ptr_id0;
    VirtualCameraImage vcam_script;
    ...
    void Start()
    {
        ...
        Calib3dBinaryWrapper.Instance.SetImageSize(1, 640, 480); // LeapMotion RenderTexture res    : VGA (640x480)
    ...
    void Update()
    {
        ...
        if (!init)
        {
            ...
            // id1: LeapMotion
            GameObject leapLeftCam = GameObject.Find("LeapLeftCamera");
            vcam_script = leapLeftCam.GetComponent<VirtualCameraImage>();

            // set intrinsic parameters for camera
            float[] cam_mat_id1 = ConvertCoord.ProjMatrix2CameraMatrix(vcam_script.GetProjMatrix(), 640, 480);
        ...
        if (find_checker)
        {
            ...
            vcam_script.RunAndCopyImage(binary_threshold_id1); // [FIX ME LATER] speed is quite slow
...
//                mOvrLeftObject3D.SetActive(false);
            ...
            if ( Calib3dBinaryWrapper.Instance.FindChecker(1, imgPts_id1) )
            {
                ...
                ConvertCoord.RT2CameraTransform(mLeapObject3D.transform, mChessboard.transform, r3x3_id1, t3x1_id1, new Vector4(0.02f, 0,0,0) );
                mLeapObject3D.SetActive(true);
            }
            else
            {
//                mLeapObject3D.SetActive(false);
            }
        }
        else
        {
        ...
        {
            Calib3dBinaryWrapper.Instance.SaveXML(filename, mLeapObject3D.transform);
        }
    }
